...
class SnapchatifyBitmapProcessor(val context: Context) {
    ...
    private val mouthBitmap: Bitmap = BitmapUtils.loadBitmap(context, null, R.drawable.mouth_2)
    private val eyeBitmapRigth: Bitmap = BitmapUtils.loadBitmap(context, null, R.drawable.eye_left)
    private val eyeBitmapLeft: Bitmap = BitmapUtils.loadBitmap(context, null, R.drawable.eye_right)
    ...
    fun annotateFace(bitmap: Bitmap, faces: List<FirebaseVisionFace>): Bitmap {
        ...
        for (face in faces) {
            ...
            if (rightEye != null){
                drawEyeRight(canvas, rightEye, eyeSize)
            }

            val leftEye = face.getLandmark(FirebaseVisionFaceLandmark.LEFT_EYE)
            if (leftEye != null){
                drawEyeLeft(canvas, leftEye, eyeSize)
            }
    ...
    private fun drawEyeRight(canvas: Canvas, eye: FirebaseVisionFaceLandmark, eyeSize: Float){
        eye.position?.let {
            canvas.withSave {
    ...
    private fun drawEyeLeft(canvas: Canvas, eye: FirebaseVisionFaceLandmark, eyeSize: Float){
        eye.position?.let {
            canvas.withSave {
                withTranslation(it.x, it.y) {
                    eyeBitmapLeft.let {
